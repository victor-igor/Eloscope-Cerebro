...
- [27/05/2026] **Maqlam — N8N search-knowledge sem agentId** · search-knowledge retorna `results:[]` porque N8N não envia `agentId` no body → todos os playbooks `private` (44 registros) são excluídos. Fix: adicionar `agentId: "767e8044-0856-4430-832f-8950b4c0da15"` no nó HTTP Request do N8N · ClickUp `86e1k160z` (high, due 28/05) · Victor
  - **[28/05/2026] Backend desbloqueado** — surgiu um 2º bug em cima: search-knowledge passou a dar **HTTP 500** porque a migration `cat05` (catálogo no RAG) renomeou o parâmetro do RPC `hybrid_search` (`p_min_similarity` → `p_max_distance`) e a edge function ainda chamava o nome antigo. Corrigido com re-deploy (v24). Teste e2e com `agentId: 767e8044...` → HTTP 200, mode hybrid, contexto populado. **Falta só a config do agentId no fluxo N8N (lado N8N, manual).**
